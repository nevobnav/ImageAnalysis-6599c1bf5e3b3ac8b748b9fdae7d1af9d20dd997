...
def clip_ortho2plot_gdal(img_path, clip_shp, out_path):
...
# =============================================================================
    ...
    try:
        input_object = gdal.Open(img_path)
    except:
...
#                        )
        ...
        warpopts = gdal.WarpOptions(format='GTiff',
                                    ...
                                    transformerOptions=['NUM_THREADS=ALL_CPUS'],
                                    cutlineDSName = shape_path,
                                    cutlineLayer = os.path.basename(clip_shp)[:-4],#'Wever_west_clipshape',
                                    )
        # perform warp operation
        ds = gdal.Warp(os.path.join(out_path, 'temp.tif'),
                       input_object,
                       options = warpopts,
